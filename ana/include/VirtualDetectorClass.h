...
class VirtualDetector{
 public:
  ...
  bool IsPDGid(float x);
  bool IsParentID(float x);
  virtual void Clear();
...
size_t VirtualDetector::GetNumOfHit(){
  return PDGid.size();
}
...
bool VirtualDetector::IsPDGid(float x){
  return InContainer(PDGid,x);
}


bool VirtualDetector::IsParentID(float x){
...
//   for(int i=0; i<this->GetNumOfHit(); ++i){
